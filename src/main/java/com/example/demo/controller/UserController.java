...
public class UserController {
	...
	@ApiImplicitParams({
			...
			@ApiImplicitParam(name = "sort", allowMultiple = true, dataType = "string", paramType = "query", value = "Sorting criteria in the format: property(,asc|desc). "
					...
					+ "Multiple sort criteria are supported.", defaultValue = "id,desc") })
	@RequestMapping(path = "/{name}", method = RequestMethod.GET)
	public Page<User> getUserByName(@PathVariable String name, @ApiIgnore @PageableDefault(size = 15, sort = {
			"createdTime" }, direction = Sort.Direction.DESC) final Pageable pageable) {
